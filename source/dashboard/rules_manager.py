...
async def receive_rules_events():
    ...
    Utilizza asyncio.wait_for per evitare di bloccare all'infinito se il broker o l'engine sono offline.
    """
    try:
        connection = await get_connection()
        async with connection:
            ...
            async with queue.iterator() as queue_iter:
                # Aspetta il prossimo messaggio per un massimo di 10 secondi
                # Utilizziamo wait_for per compatibilità con Python < 3.11
                try:
                    ...
                    return await asyncio.wait_for(get_next_msg(), timeout=10)
